...
if command -v jq >/dev/null 2>&1 && [ -f feature_list.json ]; then
  ACTIVE_FEATURE=$(jq -r '[.features[] | select(.status == "active")] | .[0].id // ""' feature_list.json 2>/dev/null || echo "")
fi
...
if [ "$MODE" = "start" ]; then
  cat > "$TRACES_DIR/session-$TIMESTAMP_FILE.json" <<EOF
{
  "session_start": "$TS",
...
EOF
  echo "Session start recorded: $TRACES_DIR/session-$TIMESTAMP_FILE.json"
  echo "Active feature: ${ACTIVE_FEATURE:-none}"

elif [ "$MODE" = "end" ]; then
  # Find the most recent session start file
  LATEST_START=$(ls -1t "$TRACES_DIR"/session-*.json 2>/dev/null | head -1)
  if [ -z "$LATEST_START" ] || ! echo "$LATEST_START" | grep -q "session-start"; then
