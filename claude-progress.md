...
- Distribution: package is publish-ready as `@maideps/ai-harness-framework` 0.4.0 (bin: create-harness, harness-upgrade, harness-audit; prepublishOnly runs check + check-arch). Publishing is set up via GitHub Actions (`.github/workflows/publish.yml`, tag `v*` or manual dispatch) using the `NPM_TOKEN` secret. Actual publish awaits the secret + user go-ahead.
- Blocker: none
